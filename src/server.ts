...
import { extractUrl, is403 } from "./forbidden.ts";
...
// Build stamp — generated by scripts/gen-build-info.ts before every compile, so
// the running exe names the exact source snapshot it came from (kills the "which
// binary is this?" ambiguity). Gitignored artefact; regenerated each build.
...
log(
  `--- start: build=${BUILD.change} (${BUILD.sha}) built=${BUILD.builtAt} ` +
    `standalone=${Deno.build.standalone} os=${Deno.build.os} data=${DATA_DIR}`,
);
...
async function runInPty(
  ...
  i: number,
  cmdline: string,
): Promise<{ code: number; forbidden: boolean; url: string | null }> {
  // Pass the command as an ARGUMENT (-Command / -lc), NOT by writing it into an
  ...
  // NOT filter the tool's output — that stays verbatim, always. ptyShell owns the
  // OS wrapping: Windows PATH refresh + `exit $LASTEXITCODE` (invisible, in the
  // -Command script); POSIX a LOGIN shell (-lc) so a Finder-launched .app sees
  // /opt/homebrew/bin (else brew/node → exit 127 "command not found").
  const { cmd, args } = ptyShell(os, cmdline);
  const pty = new Pty(cmd, { args });
  ...
  // 403 watch: decode the stream as it flows (streaming decoder handles a
  // multi-byte char split across chunks) into a running buffer, and record
  // whether a 403 ever appeared — but emit NOTHING here. winget prints
  // "Forbidden (403)" mid-stream even when it recovers internally and the
  // command still exits 0; firing the banner/browser live would flash it for a
  // 403 that resolved itself. So detection is silent; the VERDICT is left to the
  // caller (doStep), which shows the recovery UI only if the step actually
  // FAILS. The stream to the xterm stays verbatim regardless.
  ...
  try {
    while (true) {
      ...
      if (data.byteLength) {
        ...
        if (!forbidden) {
          textBuf += decoder.decode(data, { stream: true });
          if (is403(textBuf)) {
            forbidden = true;
            url = extractUrl(textBuf);
          }
        }
    ...
    return { code: pty.exitCode ?? 0, forbidden, url };
  } finally {
...
async function captureVersion(s: Step): Promise<string> {
  ...
  try {
    ...
    // shellProbe wraps it in a LOGIN shell (-lc on POSIX) so a brew-installed
    // tool's `node --version` finds `node` on a Finder-launched .app's minimal
    // PATH — same fix as the presence probe; a plain -c lost the version here.
    const probe = shellProbe(os, s.detect);
...
async function doStep(
  ws: WebSocket,
  i: number,
  action: "install" | "uninstall" | "upgrade" | "downgrade",
): Promise<boolean> {
  const s = STEPS[i];
  if (!s) return false;
  const cmd = s[action];
  ...
  try {
    ...
    const { code, forbidden, url } = await runInPty(ws, i, cmd);
    ...
    // A failed step that showed a 403 settles to `forbidden` (not plain `fail`)
    // so the row renders the firewall recovery UI. If it saw a 403 but exited ok
    // anyway (winget recovered internally), the exit code is the truth — it stays
    // ok and NO recovery UI shows. We decide at the END, never mid-stream, so a
    // self-resolving 403 never flashes the banner/browser.
    const blocked = !ok && forbidden;
    const status = ok
      ? (action === "uninstall" ? "absent" : "ok")
      : (blocked ? "forbidden" : "fail");
    ws.send(JSON.stringify({ type: "step", i, status }));
    // Only NOW — the step really failed on a 403 — surface the recovery: tell the
    // UI (banner + modal). We do NOT auto-open the blocked page here: the browser
    // window would pop over the modal before the user reads WHY it opened. The
    // page opens only when the user clicks "Open blocked page" (open-forbidden
    // handler), so the guidance is read first.
    if (blocked) {
      try {
        ws.send(JSON.stringify({ type: "forbidden", i, url }));
      } catch { /* socket closing */ }
    }
...
async function applyDiff(
  ...
  const actionOf = (i: number): string | null => {
    ...
    return actionFor(desired, {
      ...
      // A pin overrides `outdated`: actionFor compares the LIVE installed version
      // (constated by the repaint scan) to the pin → upgrade / null / downgrade.
      pin: STEPS[i].pin ?? undefined,
      installedVersion: presences[i].version ?? "",
    });
  };
  ...
  // (uninstall+install), so it never runs in the batch — it's a manual per-row
  // button only (see the row-action handler). actionFor may return "downgrade" for
  // an above-pin package; we drop it here and leave that row untouched, continuing
  // down the list (repaint-at-apply keeps it visibly above-pin). See talos-version-pin.
  type Act = "install" | "uninstall" | "upgrade";
  const AUTO_ACTS: Act[] = ["install", "uninstall", "upgrade"];
  const visualPlan = [...STEPS.keys()]
    .map((i) => ({ i, action: actionOf(i) as Act | null }))
    .filter((p): p is { i: number; action: Act } =>
      p.action !== null && AUTO_ACTS.includes(p.action)
    );
...
// Open the firewall-blocked URL for the user in a dedicated Chromium app-window
// positioned to the RIGHT of the Talos panel (so the corporate access-request
// page sits beside the terminal, not on top of it). Best-effort and silent —
// the clickable link in the row/modal banner is the fallback if this fails or
// if no URL was extractable (url === null → we don't open anything). Reuses the
// same Chromium probe as openPanel; adds a --window-position beside the panel.
function openForbiddenUrl(url: string | null) {
  if (!url) return; // nothing to open — banner degrades to a link-less message
  ...
  try {
    ...
    if (app) {
      // Place it just past the panel's right edge (panel opens at the default
      // top-left). A fixed offset is enough — exact tiling isn't the point,
      // "beside, not covering" is.
      spawn(app.cmd, [...app.args, `--window-position=${APP_WIDTH + 20},40`]);
      log(`openForbiddenUrl: app window via ${app.cmd}`);
    } else if (os === "windows") {
      spawn("cmd", ["/c", "start", "", url]);
      log("openForbiddenUrl: fallback to default browser");
    } else {
      spawn("open", [url]);
      log("openForbiddenUrl: fallback to default browser");
    }
    ...
    log(
      `openForbiddenUrl FAILED: ${(e as Error).message} — link is the fallback`,
    );
  }
}
  ...
  Deno.serve({
    ...
    if (req.headers.get("upgrade")?.toLowerCase() === "websocket") {
      ...
      socket.onopen = () => {
        ...
        socket.send(JSON.stringify({
          ...
          steps: STEPS.map((s, i) => ({
            ...
            posture: s.posture,
            isConfig: s.isConfig,
            pin: s.pin, // exact version pin (null when unpinned) — drives actionOf
          })),
          selection: readSelection(SELECTION),
          profiles: PROFILES,
          profileColumns: PROFILE_COLUMNS,
          consent: readConsent(CONSENT),
          build: BUILD, // stamp the UI with the exact source snapshot
        }));
      ...
      socket.onmessage = (ev) => {
        ...
        // Downgrade is a row-only action (never in Apply) — the sole way a user
        // brings an above-pin package down to its pin, one deliberate click.
        const ROW_ACTIONS = [
          "install",
          "uninstall",
          "upgrade",
        ...
        if (
          ...
          msg.type === "retry-step" && typeof msg.i === "number" &&
          STEPS[msg.i] && ROW_ACTIONS.includes(msg.action)
        ) {
          // Firewall-403 recovery: re-run this ONE step's original action after
          // the user cleared corporate network access in the opened page. Identical path to a
          // row action (same serial queue, same doStep, same terminal `done`) —
          // the retry IS the verification (detect, don't remember): if the 403
          // is gone the step now succeeds and the row settles ok.
          if (!ptyReady) {
            socket.send(JSON.stringify({ type: "starting" }));
            return;
          }
          const action = msg.action as RowAction;
          serialize(() => doStep(socket, msg.i, action))
            .catch((e) => log(`retry error: ${(e as Error).message}`))
            .finally(() => socket.send(JSON.stringify({ type: "done" })));
        } else if (msg.type === "open-forbidden") {
          // User clicked "Open blocked page" in the 403 recovery UI. We open on
          // demand (not auto) so the guidance is read before the window covers
          // it. Best-effort: the clickable link in the banner is the fallback.
          if (typeof msg.url === "string") openForbiddenUrl(msg.url);
