...
#   --version TAG   the release to fetch (v0.0.1-beta.38). Default: the `.talos-version`
...
#   --verify        recompute every launcher file in the kit against MANIFEST.sha256 and
#                   exit 1 on the first difference. Offline; run it on any synced replica
#
...
#   MANIFEST.sha256   one line per launcher FILE as it sits in the kit (`Talos.exe`, every
#                     file inside `Talos.app/`), in `shasum -c` format
#
...
if [ "$VERIFY" = 1 ]; then
    ...
    if shasum -a 256 -c --quiet MANIFEST.sha256; then
        echo "kit $(cat VERSION 2>/dev/null || echo '?') in $DEST: every launcher file matches its manifest"
        exit 0
    else
...
mkdir -p "$DEST"
if [ "$(cat "$DEST/VERSION" 2>/dev/null || true)" = "$TAG" ] && [ -f "$DEST/MANIFEST.sha256" ]; then
    echo "launchers already at $TAG (run with --verify to check the replica)"
else
    TMP="$(mktemp -d)"
    trap 'rm -rf "$TMP"' EXIT
    ...
    ( cd "$DEST" && { echo "$WIN_ASSET"; find Talos.app -type f; } | LC_ALL=C sort \
    ...
    printf '%s\n' "$TAG" > "$DEST/VERSION"
    {
        ...
        awk -v m="$MAC_ASSET" -v w="$WIN_ASSET" '$1 == m || $1 == w { print "  " $0 }' "$TMP/assets.txt"
        echo
        echo "files as placed: MANIFEST.sha256 — check any replica with: get-talos.sh <kit> --verify"
    } > "$DEST/KIT.txt"
    echo "launchers at $TAG (Talos.app + Talos.exe), manifest written"
fi
